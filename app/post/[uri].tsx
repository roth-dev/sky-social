...
import { ArrowLeft, MessageCircle, Heart, Repeat2, Share } from 'lucide-react-native';
...
export default function PostScreen() {
  ...
  const replies = thread?.replies?.filter((reply: any) => reply.post).map((reply: any) => reply.post) || [];

  const handleLike = async (postUri: string, cid: string, isLiked: boolean, likeUri?: string) => {
    if (!isAuthenticated) {
  ...
  const handleRepost = async (postUri: string, cid: string, isReposted: boolean, repostUri?: string) => {
    if (!isAuthenticated) {
    ...
    >
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.mainPost}>
          <Post
            post={post}
            onLike={(uri, cid) => handleLike(uri, cid, !!post.viewer?.like, post.viewer?.like)}
            onRepost={(uri, cid) => handleRepost(uri, cid, !!post.viewer?.repost, post.viewer?.repost)}
        ...
        <View style={styles.actionBar}>
          ...
          <TouchableOpacity 
            style={styles.actionButton}
            onPress={() => handleRepost(post.uri, post.cid, !!post.viewer?.repost, post.viewer?.repost)}
          >
          ...
          <TouchableOpacity 
            style={styles.actionButton}
            onPress={() => handleLike(post.uri, post.cid, !!post.viewer?.like, post.viewer?.like)}
          >
        ...
        {showReplyInput && isAuthenticated && user && (
          <View style={styles.replySection}>
            <View style={styles.replyHeader}>
              <Text style={styles.replyTitle}>Reply to @{post.author.handle}</Text>
              <TouchableOpacity 
            ...
            <View style={styles.replyInput}>
              ...
              <View style={styles.replyTextContainer}>
                ...
                <View style={styles.replyActions}>
                  ...
                  <Button
                    title={createReplyMutation.isPending ? "Replying..." : "Reply"}
                    onPress={handleReply}
        ...
        {showReplyInput && !isAuthenticated && (
          <View style={styles.authPrompt}>
            <Text style={styles.authPromptText}>Please log in to reply to posts</Text>
            <Button
              title="Log In"
              onPress={() => {
                setShowReplyInput(false);
        ...
        <View style={styles.repliesSection}>
          ...
          {replies.map((reply: any) => (
            <View key={reply.uri} style={styles.replyItem}>
              <Post
                post={reply}
                onLike={(uri, cid) => handleLike(uri, cid, !!reply.viewer?.like, reply.viewer?.like)}
                onRepost={(uri, cid) => handleRepost(uri, cid, !!reply.viewer?.repost, reply.viewer?.repost)}
                onComment={() => {}}
                isReply={true}
              />
            </View>
