...
import { X, Download, Share, MoveHorizontal as MoreHorizontal } from 'lucide-react-native';
...
export function LightBox({
  ...
  const getImageDimensions = (aspectRatio?: { width: number; height: number }) => {
    if (!aspectRatio) {
      return { width: screenWidth, height: screenHeight * 0.7 };
    }

    const imageAspectRatio = aspectRatio.width / aspectRatio.height;
    const screenAspectRatio = screenWidth / (screenHeight - HEADER_HEIGHT - FOOTER_HEIGHT);
  ...
  const panGestureHandler = useAnimatedGestureHandler({
    ...
    onActive: (event, context) => {
      if (scale.value > 1) {
        // Pan when zoomed in
        const maxTranslateX = (imageDimensions.width * scale.value - screenWidth) / 2;
        const maxTranslateY = (imageDimensions.height * scale.value - (screenHeight - HEADER_HEIGHT - FOOTER_HEIGHT)) / 2;
        ...
        opacity.value = interpolate(progress, [0, 1], [1, 0.3], Extrapolate.CLAMP);
      }
    },
    onEnd: (event) => {
      if (scale.value <= 1) {
        // Handle dismiss gesture
        const shouldDismiss = Math.abs(translateY.value) > screenHeight / 4 || 
  ...
  const doubleTapHandler = useAnimatedGestureHandler({
    onEnd: (event) => {
      if (scale.value > 1) {
        ...
        const tapY = event.y - (screenHeight - HEADER_HEIGHT - FOOTER_HEIGHT) / 2;
